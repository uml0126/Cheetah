package com.rex.cheetah.testcase.promise;
...
import com.rex.cheetah.common.promise.PromiseContext;
...
public class PromiseChainTest {
    ...
    private void step2(final Date result) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                execute(messageId, new SimpleDateFormat(CheetahConstants.DATE_FORMAT).format(result), null);
                // execute(messageId, new SimpleDateFormat(CheetahConstants.DATE_FORMAT).format(result), new IllegalArgumentException("Step2 Exception"));
            }
        }).run();
    }
}