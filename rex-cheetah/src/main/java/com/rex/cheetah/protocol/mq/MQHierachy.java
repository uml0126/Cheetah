...
public class MQHierachy {
    ...
    private void initializeSingleConnectionFactory() throws Exception {
        boolean reconnectOnException = mqPropertyEntity.getBoolean(CheetahConstants.MQ_RECONNECT_ON_EXCEPTION_ATTRIBUTE_NAME);
    ...
    private void initializeCachingConnectionFactory() throws Exception {
        int sessionCacheSize = mqPropertyEntity.getInteger(CheetahConstants.MQ_SESSION_CACHE_SIZE_ATTRIBUTE_NAME);
        boolean cacheConsumers = mqPropertyEntity.getBoolean(CheetahConstants.MQ_CACHE_CONSUMERS_ATTRIBUTE_NAME);
        boolean cacheProducers = mqPropertyEntity.getBoolean(CheetahConstants.MQ_CACHE_PRODUCERS_ATTRIBUTE_NAME);
        boolean reconnectOnException = mqPropertyEntity.getBoolean(CheetahConstants.MQ_RECONNECT_ON_EXCEPTION_ATTRIBUTE_NAME);
    ...
    private void initializePooledConnectionFactory() throws Exception {
        ...
        int maxConnections = mqPropertyEntity.getInteger(CheetahConstants.MQ_MAX_CONNECTIONS_ATTRIBUTE_NAME);
        int maximumActiveSessionPerConnection = mqPropertyEntity.getInteger(CheetahConstants.MQ_MAXIMUM_ACTIVE_SESSION_PER_CONNECTION_ATTRIBUTE_NAME);
        int idleTimeout = mqPropertyEntity.getInteger(CheetahConstants.MQ_IDLE_TIMEOUT_ATTRIBUTE_NAME);
        long expiryTimeout = mqPropertyEntity.getLong(CheetahConstants.MQ_EXPIRY_TIMEOUT_ATTRIBUTE_NAME);
        boolean blockIfSessionPoolIsFull = mqPropertyEntity.getBoolean(CheetahConstants.MQ_BLOCK_IF_SESSION_POOL_IS_FULL_ATTRIBUTE_NAME);
        long blockIfSessionPoolIsFullTimeout = mqPropertyEntity.getLong(CheetahConstants.MQ_BLOCK_IF_SESSION_POOL_IS_FULL_TIMEOUT_ATTRIBUTE_NAME);
        long timeBetweenExpirationCheckMillis = mqPropertyEntity.getLong(CheetahConstants.MQ_TIME_BETWEEN_EXPIRATION_CHECK_MILLIS_ATTRIBUTE_NAME);
        boolean createConnectionOnStartup = mqPropertyEntity.getBoolean(CheetahConstants.MQ_CREATE_CONNECTION_ON_STARTUP_ATTRIBUTE_NAME);
        boolean reconnectOnException = mqPropertyEntity.getBoolean(CheetahConstants.MQ_RECONNECT_ON_EXCEPTION_ATTRIBUTE_NAME);
    ...
    public void listen(Destination destination, SessionAwareMessageListener<BytesMessage> messageListener, String requestSelector, boolean topic) throws Exception {
        int concurrentConsumers = mqPropertyEntity.getInteger(CheetahConstants.MQ_CONCURRENT_CONSUMERS_ATTRIBUTE_NAME);
        int maxConcurrentConsumers = mqPropertyEntity.getInteger(CheetahConstants.MQ_MAX_CONCURRENT_CONSUMERS_ATTRIBUTE_NAME);
        long receiveTimeout = mqPropertyEntity.getLong(CheetahConstants.MQ_RECEIVE_TIMEOUT_ATTRIBUTE_NAME);
        long recoveryInterval = mqPropertyEntity.getLong(CheetahConstants.MQ_RECOVERY_INTERVAL_ATTRIBUTE_NAME);
        int idleConsumerLimit = mqPropertyEntity.getInteger(CheetahConstants.MQ_IDLE_CONSUMER_LIMIT_ATTRIBUTE_NAME);
        int idleTaskExecutionLimit = mqPropertyEntity.getInteger(CheetahConstants.MQ_IDLE_TASK_EXECUTION_LIMIT_ATTRIBUTE_NAME);
        int cacheLevel = mqPropertyEntity.getInteger(CheetahConstants.MQ_CACHE_LEVEL_ATTRIBUTE_NAME);
        boolean acceptMessagesWhileStopping = mqPropertyEntity.getBoolean(CheetahConstants.MQ_ACCEPT_MESSAGES_WHILE_STOPPING_ATTRIBUTE_NAME);
