...
public class NettyClientExecutor extends AbstractClientExecutor {
    ...
    public void start(String interfaze, ApplicationEntity applicationEntity) throws Exception {
        ...
        barrier.await(properties.getLong(CheetahConstants.NETTY_CONNECT_TIMEOUT_ATTRIBUTE_NAME) * 2, TimeUnit.MILLISECONDS);
    }
    ...
    private void connect(final String interfaze, final ApplicationEntity applicationEntity, final CyclicBarrier barrier) throws Exception {
        ...
        final EventLoopGroup group = new NioEventLoopGroup(CheetahConstants.CPUS);
            ...
            public Object call() throws Exception {
                try {
                    ...
                    client.group(group).channel(NioSocketChannel.class)
                            ...
                            .option(ChannelOption.CONNECT_TIMEOUT_MILLIS, properties.getInteger(CheetahConstants.NETTY_CONNECT_TIMEOUT_ATTRIBUTE_NAME))
                            .option(ChannelOption.ALLOCATOR, PooledByteBufAllocator.DEFAULT)
                            .option(ChannelOption.WRITE_BUFFER_LOW_WATER_MARK, properties.getInteger(CheetahConstants.NETTY_WRITE_BUFFER_LOW_WATER_MARK_ATTRIBUTE_NAME))
                            .option(ChannelOption.WRITE_BUFFER_HIGH_WATER_MARK, properties.getInteger(CheetahConstants.NETTY_WRITE_BUFFER_HIGH_WATER_MARK_ATTRIBUTE_NAME))
                            .handler(new LoggingHandler(LogLevel.INFO))
                            .handler(new ChannelInitializer<SocketChannel>() {
                                @Override
                                public void initChannel(SocketChannel channel) throws Exception {
                                    channel.pipeline()
                                           .addLast(new IdleStateHandler(properties.getLong(CheetahConstants.NETTY_WRITE_IDLE_TIME_ATTRIBUTE_NAME), properties.getLong(CheetahConstants.NETTY_READ_IDLE_TIME_ATTRIBUTE_NAME), properties.getLong(CheetahConstants.NETTY_ALL_IDLE_TIME_ATTRIBUTE_NAME), TimeUnit.MILLISECONDS))
                                           ...
                                           .addLast(new WriteTimeoutHandler(properties.getInteger(CheetahConstants.NETTY_WRITE_TIMEOUT_ATTRIBUTE_NAME)))
                                           .addLast(new ReadTimeoutHandler(properties.getInteger(CheetahConstants.NETTY_READ_TIMEOUT_ATTRIBUTE_NAME)))
                                           .addLast(new NettyClientHandler(executorContainer, properties.getBoolean(CheetahConstants.TRANSPORT_LOG_PRINT_ATTRIBUTE_NAME), properties.getBoolean(CheetahConstants.HEART_BEAT_LOG_PRINT_ATTRIBUTE_NAME)));
                                }
                            });
                    ...
                    TimeUnit.MILLISECONDS.sleep(properties.getInteger(CheetahConstants.NETTY_RECONNECT_DELAY_ATTRIBUTE_NAME));
                    ...
                    boolean started = started(null, applicationEntity);
                    if (online && !started) {
                        LOG.info("Channel for {}:{} is closed, try to reconnect...", host, port);
